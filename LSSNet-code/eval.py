...
def compute_score(pred_root, gt_root, sample, Thresholds):
    ...
    for j, threshold in enumerate(Thresholds):
        ...
        # PreFtem, RecallFtem, SpecifTem, Dice, FmeasureF, IoU
...
def evaluate(result_path, pred_root_ori, gt_root_ori, verbose=False, debug_dataset_len=5):
    ...
    Thresholds = [0.5]  # np.linspace(1, 0, 256)从0-255对应到0-1    #
    ...
    for dataset in datasets:
        ...
        if verbose is True:
            samples = tqdm.tqdm(enumerate(zip(preds, gts)), desc=dataset + ' - Evaluation',
                                     total=len(
        ...
        csv.write(out_str)
        csv.close()
    tab = tabulate(results, headers=['dataset', *headers], floatfmt=".4f")
...
if __name__ == "__main__":
    result_path = "result_path"     # Save the result paths of various indicators
    pred_root = "pred_root"  # Save path for prediction maps
    gt_root = "E:\\notes\\dataset\\polyp\\TestDataset"  # Saving path of GT
